...
bool ModelClass::Initialize(ID3D11Device* device, char* modelFilename, WCHAR* textureFilename , WCHAR* normalTextureFilename)
{
	bool result;
	...
	// Load the texture for this model.
	result = LoadTextures(device, textureFilename, normalTextureFilename);
	if(!result)
	{
		return false;
	}
...
{
	ID3D11ShaderResourceView** textures = m_TextureArray->GetTextureArray();
...
bool ModelClass::LoadTexture(ID3D11Device* device, WCHAR* filename)
{
	...
	result = m_TextureArray->Initialize(device, filename, NULL);
	if(!result)
	{
		return false;
	}
